...
- add_comment / create_issue / change_status / change_assignee / update_due_date / update_priority / update_done_ratio / assign_version / add_relation は「確認待ち」のツールで、呼び出しても即時には実行されず、ユーザーが承認するための提案カードが表示される。テキストで確認を求めるのではなく、ツールを呼び出して提案カードを出すこと
