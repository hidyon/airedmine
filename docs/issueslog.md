...
## 2026-07-30: フルスタック実操作で見つけた 2 件の修正を issue 化（ISS-138 / ISS-139）

背景:

- README 見直し・デモスクリーンショット拡充の作業中に、実アプリを `docker compose` で起動して手動操作した。
- その過程で M31 の範囲外の不具合を 2 件発見・修正したが、issue として追跡されていなかった。突発対応でも記録を残す方針に沿い、Closed issue として遡及記録した。

判断:

- 2 件とも「デモ環境の説得力・初回体験の信頼性」に直結し、当面のロードマップ方針（実運用検証より先にデモの説得力と計測可能性を高める）と整合するため記録価値が高いと判断した。
- 既に実装・検証・コミット済みのため、新規着手ではなく Closed として要求 / 機能 / テスト仕様と検証結果を残した。

記録した issue:

- `ISS-138`: AI Agent の tool_use ループが `max_tokens` 途中終了で 400 になる不具合を修正（commit `b91c654`）。空の `tool_result` を積まず途中回答を返すガードと回帰テストを追加。
- `ISS-139`: クイックスタートの順序を修正し意味検索を初回から使えるようにする（commit `a21b4db`）。`seed:demo:no-index` → キー設定 + 再起動 → index build の順に再構成し、`.env.example` の `REDMINE_API_KEY` を空にした。誤順序 8 件（mock）/ 正順序 517 件（デモ issue）を実機で再現確認。
...
## 2026-07-30: ISS-140 クローズ — seed の期日設定

PM Dashboard の期限切れが常に 0 件だった原因は、seed が `due_date` を設定していなかったこと。

判断:

- 期日は所属 sprint の期日に連動させる方針にした。過去スプリントのやり残し（open）が期限切れになる、という現実的なモデルになるため。
- 「期限切れの量」は現実的方針を採用（ユーザー選択）。進行中スプリントの作業は期日前なので大量には出ない。健全なプロジェクトの姿として妥当。
- 逆に「デモ映えのため大量に出す」案は不採用。必要になれば `due_days_from_now` の個別指定で troubled scenario を作れる余地は残した。
...
## 2026-08-01: M31 追加 issue の検討（ISS-146〜150）

M31 第一弾（ISS-133〜137）は完了。同テーマ「日常利用の迷いを減らす」で残る摩擦を、今セッションの実装・検証で見えた点から洗い出した。

候補と比較（迷いの種類 → 施策）:

- 「なぜこの回答か / 回答から issue へ移動」→ **ISS-146**（root references のライブ表示）。実装調査で `run_agent` が references を返さず、参照チップが fixture のみと判明した実ギャップ。価値が高く High。
- 「更新提案を実行してよいか」→ **ISS-148**（現在値→変更後の差分）。実行判断の材料を増やす。危険操作の二段階確認は不変。
- 「実行後、文脈へ戻る」→ **ISS-147**（Audit → issue / セッション導線）。ループを閉じる。
- 「どの issue から、どう動くか」→ **ISS-149**（詳細/行からのクイック提案）。ISS-135 の拡張。
- 「何を聞けばよいか」→ **ISS-150**（動的質問候補）。静的候補を実データ連動に。Low。
