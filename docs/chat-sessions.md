...
セッションが増えたとき、過去の相談を素早く見つけられるようにする。既存の通常 / 全履歴切替・アーカイブ・リネームと矛盾しない、軽量な絞り込みを初期スコープとする。

検索対象候補の比較:

| 対象 | 初期判断 | 理由 |
| --- | --- | --- |
| title | 採用 | 最も探しやすい。リネームで内容に合わせられるため実効性が高い |
| related issue IDs | 採用 | 「あの issue の相談」を `#NNN` から辿れる。metadata に既にある |
| last proposal action | 見送り | 種類が少なくファセット向き。絞り込み UI が増えるので後続で検討 |
| message text | 後続 issue | 全文検索は SQLite FTS などサーバー側実装が必要。別 issue に分割 |

初期実装（ISS-137）:

- Chat サイドバーに絞り込み入力を追加し、**title の部分一致**または**関連 issue ID の一致**でクライアント側フィルタする。
- 対象は現在表示中の一覧（通常 / 全履歴の切替に従う）。アーカイブ済みを検索対象に含めるかは、この切替で制御する（別トグルは増やさない）。
- 読み込み済みの session に対するフィルタとし、サーバー検索は導入しない。

後続 issue 候補:

- **message text の全文検索**（サーバー側 FTS。session 数・message 量が増えたときに評価）。
- last proposal action / role でのファセット絞り込み。
- session 数が大きくなった場合のサーバー側検索・ページング。
