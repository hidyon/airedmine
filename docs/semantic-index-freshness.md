...
`GET /api/ai/index/freshness` は次の情報を返す。
...
実装上は、同じ issue_id が Redmine に存在し、Redmine `updated_on` が `issue_embeddings.indexed_at` より新しいものを stale issue として扱う。
また、`issue_embeddings.issue_id` が Redmine の現在 issue 一覧に存在しないものを orphan embedding として扱う。

主なレスポンス項目:

- `indexed_count`: `issue_embeddings` に保存されている件数
- `current_count`: Redmine から取得した現在の issue 件数
- `fresh_count`: indexed issue のうち Redmine より古くない件数
- `stale_count`: Redmine 側の `updated_on` が `indexed_at` より新しい件数
- `orphan_count`: Redmine に存在しない indexed issue 件数
- `missing_count`: Redmine に存在するが index に存在しない issue 件数
- `samples.stale`: stale issue の代表サンプル
- `samples.orphan`: orphan embedding の代表サンプル
- `samples.missing`: missing issue の代表サンプル

`stale_count` または `orphan_count` が大きい場合は、`POST /api/ai/index/build` で全体を再構築する。
この API は検出専用であり、自動再構築や issue 単位再構築は行わない。

### embedding 対象の再検討

コメント履歴や説明を意味検索に含める場合、更新影響は大きくなる。
検索品質は上がる可能性があるが、コメント追加のたびに再 embedding が必要になる。
embedding 対象の候補比較と推奨 v1 は [`semantic-embedding-scope.md`](semantic-embedding-scope.md) に整理した。

## 推奨

1. ISS-105 で seed 後の全再構築導線を追加する。
2. ISS-107 で stale / orphan 検出 API を追加した。
