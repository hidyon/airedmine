...
1. `_issue_text()` / `_issue_body()` を推奨 v1 に拡張する。
2. index build 時に issue detail を取得し、description と直近コメントを含める。
3. 代表質問セットを固定し、検索結果の上位 issue を記録する評価スクリプトを追加する。
4. index build の detail fetch / encode / write の計測を追加し、500 件規模での許容時間を確認する。
