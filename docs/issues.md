...
- README に「デモで試す質問例」セクションを追加した。
- seed データ投入後に試せる Chat の開発者向け、PM 向け、意味検索向けの質問例を記載した。
- 更新提案から Audit へつなげる質問例を追加した。
- Dashboard で見るべき担当 issue、ブロッカー、担当者負荷、停滞 issue、期限切れ、優先度偏りを明記した。
- README の seed 投入手順から代表質問、意味検索インデックス構築、Audit 確認まで辿れることを確認した。
...
- Chat を一問一答ではなく、相談トピック単位のセッションとして扱えるようにするための体験要件を整理する。
- 開発者、PM、更新提案確認のそれぞれで、どの単位を 1 セッションとみなすかを決める。
- 対象外: この issue では API や UI の実装は行わない。

機能仕様:

- セッション名、作成日時、最終更新日時、参加ロール、関連 issue、proposal / audit log との紐付け候補を整理する。
- セッションを再開したときに表示すべき情報と、AI に渡すべき文脈を分けて定義する。
- セッション削除、アーカイブ、検索、ピン留めの必要性を初期スコープと後続候補に分ける。

テスト仕様:

- 代表ユースケースとして「今日の担当 issue 相談」「Sprint 3 リリース判断」「更新提案の確認」をセッション単位で記述する。
- 後続の API / UI issue に切れる粒度で要件を記録する。

実装結果:

- `docs/chat-sessions.md` を追加し、現状、セッション単位、metadata、UI 表示と AI 文脈の分離、初期スコープ、後続 issue への分割を整理した。
- 初期スコープでは、1 セッションを「1 つの相談トピック」とみなし、今日の担当 issue 相談、Sprint 3 リリース判断、更新提案の確認、調査・深掘りを代表ユースケースとした。
- AI へ渡す履歴は同一 `session_id` の直近 message に限定し、別セッション混入を避ける方針とした。
- セッション検索、ピン留め、アーカイブ、削除 UI、LLM による title 生成、長期記憶は初期スコープ外とした。
...
- Chat UI がセッション一覧を表示し、既存セッションの履歴を取得できる API を追加する。
- 既存の `conversations` テーブルとの関係を整理し、必要なら session metadata を保存する。

機能仕様:

- セッション一覧 API を追加し、session_id、title、role、created_at、updated_at、message_count を返す。
- セッション詳細 API を追加し、該当 session_id の messages を時系列で返す。
- `POST /api/chat` が受け取る session_id と保存される履歴の関係を明確にする。
- セッション title は初期は最初の user message から生成するなど、軽いルールで決める。
...
- ユーザーが Chat 画面で新しいセッションを開始し、過去セッションに切り替え、履歴を見ながら会話を再開できるようにする。

機能仕様:

- Chat 画面にセッション一覧またはセッション切替 UI を追加する。
- 新規セッション作成、既存セッション選択、現在セッションの表示を実装する。
- セッション履歴を読み込み、ユーザー発話と AI 応答を時系列で表示する。
- モバイル幅でも会話領域とセッション切替が破綻しないようにする。
...
- セッション履歴を AI に渡すとき、必要な文脈は維持しつつ、古い話題や別セッションの情報が混ざらないようにする。
- 対象外: 長期記憶、ユーザー横断の記憶、LLM による自動要約の高度化。

機能仕様:

- `POST /api/chat` で同じ session_id の直近 messages を AI 文脈に含める。
- 文脈に含める件数または文字数の上限を決める。
- 更新提案や tool result をどこまで文脈に含めるかを整理する。
- セッション履歴が多い場合の切り詰め方を実装する。
...
- 初見ユーザーや久しぶりに使うユーザーが、Chat で最初に何を聞けばよいか迷いにくくする。
- developer / pm のロールごとに、現在のデモシナリオや実作業に近い質問候補を選びやすくする。
- 対象外: AI 回答品質そのものの改善、Anthropic prompt の大幅変更。

機能仕様:

- Chat の空状態または新規 session 作成時に、ロール別の質問候補を提示する。
- 候補は README のデモ質問例、seed シナリオ、代表 issue と矛盾しない内容にする。
- 候補を選ぶと入力欄に反映するか、そのまま送信できる導線を検討する。

テスト仕様:

- developer / pm それぞれで質問候補が表示されることをブラウザまたは smoke test で確認する。
- README のデモ質問例と候補が大きく矛盾しないことを確認する。
...
- ユーザーが AI の更新 proposal を実行した後、次に何を確認すればよいか迷わないようにする。
- 成功時は Audit 確認や関連 issue の再確認へ進みやすくし、失敗時は原因と再試行可否を見つけやすくする。
- 対象外: Redmine 更新 API の新規操作追加、Audit log の保存方式変更。

機能仕様:

- Proposal 実行後のカードまたは通知に、実行結果、Audit への導線、関連 issue 詳細への導線を出す。
- 失敗時は category / retryable / HTTP status がある場合に短く表示する。
- 危険操作や一括更新では、対象件数や確認済みであることが実行後にも分かるようにする。

テスト仕様:

- 成功 proposal 実行後に Audit / issue 詳細へ進めることを確認する。
- 失敗 proposal の固定レスポンスまたは既存テストで、原因表示が崩れないことを確認する。
...
- issue 詳細パネルを読んだ後に、関連する質問、更新 proposal、Dashboard 上の文脈へ移りやすくする。
- 開発者が issue の説明や journals を読んでから、次の相談や更新依頼に自然につなげられるようにする。
- 対象外: issue 詳細 API のデータ構造変更、Redmine 直接編集フォームの追加。

機能仕様:

- issue 詳細パネルに「この issue について Chat で相談する」などの導線を検討する。
- Dashboard から開いた場合も、issue ID と件名を保持して Chat に渡せるか検討する。
- 既存の `#NNN` 詳細リンクと競合しない導線にする。

テスト仕様:

- Chat / Dashboard から issue 詳細を開いたあと、関連する Chat 操作へ進めることを確認する。
- 既存の issue 詳細パネル表示が壊れていないことを smoke test または手動で確認する。
...
- PM が Dashboard で見つけた停滞 issue、期限切れ issue、優先度偏りを、次の定例で話す内容に変換しやすくする。
- 画面上のリスク一覧を見るだけで終わらず、Chat での相談やアジェンダ作成につなげる。
- 対象外: 本格的な議事録生成、外部カレンダー連携。

機能仕様:

- PM Dashboard から、現在表示されているリスクを Chat に持ち込む導線を検討する。
- 初期実装では固定文面の質問作成または入力欄への反映を候補とする。
- 対象 issue 数や代表 issue ID が分かる形にする。

テスト仕様:

- PM Dashboard で停滞 / 期限切れ issue がある状態から、Chat へ相談文を渡せることを確認する。
- `npm run smoke:demo` の PM Dashboard 確認観点を必要に応じて更新する。
...
- Chat session が増えたときに、過去の相談を探しやすくする改善候補を整理する。
- 既存の通常 / 全履歴切替、アーカイブ、リネームと矛盾しない検索・絞り込み方針を決める。
- 対象外: 全文検索エンジン導入、session 削除機能の再検討。

機能仕様:

- 検索対象候補として title、related issue IDs、last proposal action、message text を比較する。
- 初期実装候補として title / issue ID の軽量フィルタを検討する。
- アーカイブ済み session を検索対象に含めるかを整理する。

テスト仕様:

- `docs/chat-sessions.md` または関連 docs に検索・絞り込み方針が記録されていることを確認する。
- 後続実装 issue に分けられる粒度になっていることを確認する。
