...
終了したマイルストーンと過去の変更履歴を保管する。現在進行中のロードマップは [`docs/roadmap.md`](roadmap.md) を参照する。
...
背景: M22 で README / docs 用スクリーンショットを更新したが、撮影手順は一時スクリプトと手元環境に依存していた。
UI の変更に追従してスクリーンショットを更新し続けるため、代表状態を安定して再現できる撮影コマンドを追加した。

関連 issue:

- `ISS-127` Closed: README 用スクリーンショット撮影をコマンド化する。

期待成果:

- Chat / PM Dashboard / Audit の代表状態を同じデータで再撮影できる。
- README のスクリーンショット更新手順が属人化しない。

完了条件:

- `npm run screenshots` で `docs/screenshots/*.png` を更新できる。
- 撮影に必要な前提とコマンドが README から分かる。
- issue に検証結果が記録されている。
...
背景: M23 で、session 整理操作はリネームとアーカイブを初期実装候補、削除を当面対象外とする方針にした。
履歴を消さずに見つけやすくする最小操作として、手動リネームとアーカイブを実装した。

関連 issue:

- `ISS-122` Closed: Chat session title を手動リネームできるようにする。
- `ISS-123` Closed: Chat session をアーカイブして通常一覧から隠せるようにする。

期待成果:

- 最初の質問から自動生成された title が相談内容とずれても、ユーザーが自分で分かりやすく直せる。
- 完了した session を通常一覧から隠し、履歴を消さずに整理できる。

完了条件:

- ISS-122〜123 が完了し、セッション履歴を消さずに整理できる最小操作が揃っている。
...
背景: 現状の Chat は照会・要約と、コメント/ステータス/担当変更の 3 操作のみ実行できる状態から始まった。
「起票する」「期日を設定する」などの操作もチャットから完結できるようにすることで、Redmine を直接開く場面をさらに減らした。
...
- `ISS-084` Closed: チャットから複数 issue を一括操作できるようにする。
...
Redmine issue の探索、PM 判断、更新提案は一問一答では完結しにくいため、会話をセッション単位で保存・再開・整理できるようにした。
...
- `ISS-114` Closed: セッション履歴を AI の文脈に安全に渡す方針を実装する。
...
## Milestone 16: MCP ツール拡充

優先度: Medium
状態: Completed

背景: ISS-085 で Redmine MCP サーバーの基盤（照会・作成・更新の 7 ツール）が動いた。実利用で、名前から ID を解決できない摩擦と、更新できる項目が限られる点が見えた。M14（チャット側の操作拡張）と対になる形で、MCP 側の操作種別も増やす。

関連 issue:

- `ISS-086` Closed: MCP に参照系ヘルパーツールを追加する（projects / statuses / priorities / users / versions の一覧）。
- `ISS-087` Closed: MCP に更新系ツールを追加する（期日・優先度・進捗率・バージョン割当・関連付け）。

期待成果:

- Claude Code から名前で対象を指定でき（ID を人が調べる必要がない）、主要な Redmine 更新操作が一通り実行できる。

完了条件:

- ISS-086・087 がクローズされ、MCP 経由で ID 解決と主要更新操作が動作する。
...
ISS-084 の一括操作は誤操作リスクが高いため、このマイルストーンで安全性・監査性の土台を強化した後に再判断する。
...
- Redmine へ書き込む前に、変更前後・対象・理由を操作種別に応じて確認できる。
- PM や開発者が Audit から実行履歴を探し、失敗原因や再試行可否を判断できる。
- AI が優先度・担当者・バージョンなどの ID を推測せず、参照ツールに基づいて提案を作れる。
- 危険な変更は通常操作より明確に確認され、誤操作への不安を減らせる。

完了条件:

- Proposal カードで主要更新操作の差分または変更内容が操作種別ごとに表示される。
- Audit View で操作種別・結果・issue_id による絞り込みができる。
- 更新失敗時に Redmine API のエラー内容、再試行可否、次に取る行動が分かる。
...
- 2026-06-13: Milestone 17 を追加。M14 で拡張した Redmine 書き込み操作を、安全に確認・監査・再試行できる体験へ磨く方針とした。ISS-088〜092 を候補 issue として追加。
- 2026-06-13: ISS-088 を完了。Proposal カードに操作種別ごとの確認項目表示を追加し、対象・変更内容・変更後・理由を実行前に確認できるようにした。
...
- 2026-06-14: ISS-111〜114 を完了。チャットセッション要件、一覧・詳細 API、セッション切替 UI、保存済み履歴の AI 文脈投入を実装し、M21 を Completed とした。
- 2026-06-14: ISS-084 を完了。Chat に bulk_update 提案、`/api/proposals/bulk_update`、ProposalCard の一括更新表示と追加確認を追加し、M14 を Completed とした。
- 2026-06-08: ISS-079 を完了し、チャットから issue を新規作成できるようにした（提案 → 確認 → 実行フロー）。M14 の最初の issue。
- 2026-06-13: ISS-080〜083 を完了し、チャットから期日・優先度・進捗率・バージョン割当・関連付けを提案 → 確認 → 実行できるようにした。`npm run build` と backend tests 19 件、ブラウザ起動確認が成功。一括操作は誤操作リスクが高いため、この時点では後続対応にした。
