...
- `ISS-105` Open: seed 再投入後に semantic index を洗い替える。
- `ISS-106` Closed: issue 更新が semantic index に与える影響を分析する。
- `ISS-107` Open: semantic index の stale / orphan 状態を検出する。
...
- ISS-102〜107 の結果が `docs/performance.md` または関連ドキュメントに追記され、次に最適化すべき箇所が更新されている。
