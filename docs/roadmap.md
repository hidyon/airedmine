...
ステータス変更や担当変更の実行フローが未完成のため、体験の仕上げとして実施する。

関連 issue:

- `ISS-071` Closed: Chat 回答をマークダウンレンダリングする。
- `ISS-072` Open: PM View を Claude Agent に刷新する。
- `ISS-073` Open: ステータス変更・担当変更の実行フローを実装する。
- `ISS-074` Open: バックエンドのユーザー認証基盤を実装する。
- `ISS-075` Open: フロントエンドにログイン画面とセッション管理を追加する。

期待成果:

- Claude の回答が箇条書き・コードブロックを含むマークダウンとして正しく表示される。
- PM が PM View から直接 AI の分析を受け取れる。
- コメント追加に加えて、ステータス変更と担当変更も確認後に実 Redmine へ反映できる。

完了条件:

- Chat の回答バブルでマークダウンがレンダリングされる。
- PM View に「AI 分析を取得」ボタンがあり、Claude が Redmine を参照した分析を返す。
