...
| View | URL | 対象 | 概要 |
| --- | --- | --- | --- |
| Developer Chat | `/developer/chat` | 開発者・PM | Claude と対話しながら issue を探索・更新案を作成するスレッド型チャット |
...
- **ロール別回答**: developer（技術的な優先順位・ブロッカー）と pm（リスク・停滞・全体状況）で回答の切り口が変わる
- **意味検索**: キーワードが一致しなくても意味的に近い issue を発見できる（sentence-transformers）
- **Redmine コメントの確認・実行**: AI が作成したコメント案を人間が確認してから Redmine に送信
- **担当 issue 一覧と詳細確認**: Dashboard で issue をクリックして説明・コメント履歴を確認
- **PM サマリー**: 停滞 issue・高優先度・PM 判断待ちをカードで一覧
